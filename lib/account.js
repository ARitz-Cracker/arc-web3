...
class EthereumAccount {
	...
	async signData(data){
		...
		const fullSig = await this._connection.sign(this.address, binaryData);
		const hash = keccak256(Buffer.concat([
			Buffer.from("\x19Ethereum Signed Message:\n" + binaryData.length.toString(), "ascii"),
			binaryData
		]), false);
		return {
			messageHash: hash,
	...
	transfer(account, amount, gasPrice, gasAmount, nonce){
	...
	async sendTransaction(txData){
		...
		if(txData.gas == null){
			txData.gas = await this._connection.estimateGas({
				from: this.address,
				to: txData.to,
				data: txData.data,
			...
			if(txData.gas > 21000){
				txData.gas *= 1.25;
				txData.gas |= 0;
				if(txData.gas > GAS_LIMIT){
					txData.gas = GAS_LIMIT;
				}
			}
		}
