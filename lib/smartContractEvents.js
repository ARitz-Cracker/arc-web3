...
class EthereumContractEventEmitter { // Implements NodeJS's eventEmitter
	constructor(contract, abi){
		...
		for(let i = 0; i < abi.length; i += 1){
			...
			for(let ii = 0; ii < abiSnip.inputs.length; ii += 1){
				const type = abiSnip.inputs[ii].type;
				funcSelector += type + ",";
			}
			...
			const indexedInputs = [];
			for(let ii = 0; ii < abiSnip.inputs.length; ii += 1){
				if(abiSnip.inputs[ii].indexed){
					indexedInputs.push(abiSnip.inputs[ii]);
				}else{
					inputs.push(abiSnip.inputs[ii]);
				}
			}
		...
		for(let i = 1; i < topics.length; i += 1){
			if(Array.isArray(topics[i])){
				for(let ii = 0; ii < topics[i].length; ii += 1){
					topics[i][ii] = this._verifyTopic(topics[i][ii], indexedInputs[i].type);
				}
			}else{
				topics[i] = this._verifyTopic(topics[i], indexedInputs[i].type);
			}
		}
		return topics;
	}
	...
	blockRange(...args){
		...
		this.contract._jsproperties.connection.listen(
