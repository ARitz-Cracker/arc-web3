...
const {decodeString} = require("./ABIDecoder.js");
...
class Web3Connection{
	...
	async _tick(){
		...
		await Promise.all(Object.keys(this._promisedTransactions).map((txHash) => {
			return (async() => {
	...
	async transactionConfirmed(txHash){
		...
		if (this._promisedTransactions[txHash] === undefined){
			...
			p.resolve = p_resolve; // This really should be standard.
			p.reject = p_reject;
	...
	async switchTransactionConfirmed(oldTxHash, newTxHash){
		const p = this.transactionConfirmed(newTxHash);
		if (this._promisedTransactions[oldTxHash] !== undefined){
			this._promisedTransactions[oldTxHash].resolve(p);
