...
const checkChecksumAddress = function(address){
    ...
    for (let i = 0; i < 40; i += 2) {
        if (addressHash.charCodeAt(i) > 55) {
            ...
            if (address.charCodeAt(i) > 64 && address.charCodeAt(i) < 91){ // Is upper case
                return false;
            } 
        }
    }
...
const isValidAddress = function(address, lenient){
    ...
    if (lenient){
        return /^(0x|0X)?[0-9a-f]{40}$/.test(address) || /^(0x|0X)?[0-9A-F]{40}$/.test(address) || checkChecksumAddress(address);
    }else{
