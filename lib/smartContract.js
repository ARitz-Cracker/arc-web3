	...
	constructor(contract,abiIndex){
		...
		if (abiSnippet.stateMutability == "pure" || abiSnippet.stateMutability == "view"){
			super((...args) => { // By using arrow functions, "this" becomes the function itself
				return this._contract._wrapperFuncRead(this._i,args,this.selectorHash);
			});
		}else{
			super((...args) => {
				return this._contract._wrapperFuncWrite(this._i,args,this.selectorHash);
			});
		}
		...
		this._contract[this.selector] = this;
		this._contract[this.selectorHash] = this;
	}
	call(...args){
		return this._contract._wrapperFuncRead(this._i,args,this.selectorHash);
	}
	...
	constructor(contract,funcArgs,funcName){
		super(async (...args) => { // Even though no async operations are done here, thrown errors should still be rejected promises.
			...
			if (possible.length == 0){
				throw new Error(contract._jsproperties.account.address+"."+funcName+": Expected amount of arguments to be "+minVars+" <= x <="+maxVars+", got "+args.length);
			}
			...
			for (let i=0;i<args.length;i+=1){
				if (args[i] == null){
					throw new Error(contract._jsproperties.account.address+"."+funcName+": null or undefined arguments are not allowed.");
				}
			...
			if (possible.length == 0){
				throw new Error(contract._jsproperties.account.address+"."+funcName+": Type mismatch."); // I really hate every time I write Something Happened™
			}
			if (possible.length == 1){
				return possible[0]._f(...args);
			}
			...
			if (possible.length == 0){
				throw new Error(contract._jsproperties.account.address+"."+funcName+": Array length mismatch."); // I really hate every time I write Something Happened™
			}
			if (possible.length == 1){
				return possible[0]._f(...args);
			}
			...
			if (anyPriority){
				...
				if (possible.length == 0){
					throw new Error(contract._jsproperties.account.address+"."+funcName+": This shouldn't happen? (There was and wasn't matching fixed length arrays at the same time.)");
				}
			}
			...
			throw new Error(contract._jsproperties.account.address+"."+funcName+": There are multiple functions with the same name; Unable to determine which function to use with the given arguments.");
...
class EthereumContract{
	...
	async _wrapperFuncRead(index,args,selectorHash){
		let abiSnippet = this._jsproperties.abi[index];
		...
		if (options.from instanceof EthereumAccount){
			...
			finalOptions.from = options.from; // TODO: Add type checking, address string is assumed here.
		}
	...
	async _wrapperFuncWrite(index,args,selectorHash){
		throw new Error("Writing contracts isn't a thing here right now.");
	}
}
