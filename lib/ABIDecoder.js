...
let deocdeFixedBuffer = function(hexStr,offset,byteLength){
	return buffConvert.hexToBuffer(hexStr.substring(offset,offset+byteLength*2),false);
}
...
let decodeBuffer = function(hexStr,offset){
	...
	if (len > 0){
		...
		return buffConvert.hexToBuffer(hexStr.substring(stringPos,endPos),false);
	}else{
		return buffConvert.newBufferUnsafe(0);
	}
}
...
let decodeString = function(hexStr,offset){
	...
	if (len > 0){
		...
		return buffConvert.hexToString(hexStr.substring(stringPos,endPos),false);
	}else{
		return "";
	}
}
