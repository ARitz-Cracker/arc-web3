...
let encodeAddress = function(thing){
	if (typeof thing == "string"){
		if (/^0x[0-9a-f]{40}$/i.test(thing)){ 
			// TODO: do checksum testing.
			return "0".repeat(24)+thing.substring(2).toLowerCase();
		}else{
			//throw new TypeError("Invalid 'address' argument.");
			return nullArgument;
		}
	}else if(thing instanceof EthereumContract){
		return "0".repeat(24)+thing._jsproperties.account.address.substring(2).toLowerCase();
	}else if(thing instanceof EthereumAccount){
...
let encodeFunction = function(contractFunc){
	if (contractFunc instanceof EthereumContractMultiFunction){
		throw new TypeError("This EthereumContract has multiple functions with the same name. You must select the function using its name and type.");
	}
	...
	return contractFunc._contract._jsproperties.account.address.substring(2).toLowerCase()+contractFunc.selectorHash.substring(2)+("0".repeat(16));
}
let encodeFixedBuffer = function(buff,len){
	...
	let str = buffConvert.bufferToHex(buff,false);
	return str+("0".repeat(64-str.length));
}
let encodeBuffer = function(buff){
	if (!(buff instanceof Uint8Array)){
		buff = buffConvert.newBufferUnsafe(0);
	}
	let hexstr = buffConvert.bufferToHex(buff,false);
...
let encodeString = function(str){
	...
	str = String(str);
	let hexstr = buffConvert.stringToHex(str,false);
	let len = hexstr.length / 2;
	let finalResult;
	if (len == 0){
