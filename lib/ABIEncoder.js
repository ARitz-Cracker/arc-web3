/*
   ...
   These cannot be required here, otherwise dependencies will be circular */
const classes = {};
const getClassDefinitionsAtRuntime = function(){
	if(classes.EthereumAccount == null){
		Object.assign(classes, require("./account.js"));
		Object.assign(classes, require("./smartContract.js"));
	}
}
...
const encodeAddress = function(thing){
	...
	if(typeof thing == "string"){
		...
		return nullArgument;
	}else if(thing instanceof classes.EthereumAccount || thing instanceof classes.EthereumContract){
...
const encodeArguments = function(args, types){
	...
	for(let i = 0; i < pendingNullEncodes.length; i += 1){
		const startIndex = pendingNullEncodes[i];
		const endIndex = pendingNullEncodes[i] + 64;
		// Replace placeholder guard bytes with pointer to a zero
		hexStr = hexStr.substring(0, startIndex) + dynamicNullOffsetHex + hexStr.substring(endIndex);
	}
	return hexStr;
};
