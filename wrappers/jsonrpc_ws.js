...
class WSJSONRPCer {
	...
	_socketMessage(msg){
		...
		try{
			...
			const err = new Web3ConnectionError("The node isn't returning valid JSON", -32700, msg);
	...
	async _actuallyDoRequests(){
		...
		while(this._promises.length > 0){
			const params = this._promises.shift();
			const finalResolve = params.resolve;
			const finalReject = params.reject;
			...
			try{
				...
				finalResolve(await new Promise((resolve, reject) => {
